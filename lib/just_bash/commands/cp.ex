defmodule JustBash.Commands.Cp do
  ...
  # The `:enotdir` note in copy_dir/4 called this wording unverified because
  # telling "destination exists as a regular file" from "an *ancestor* is a
  # regular file" needed an lstat first. It no longer does: component-wise path
  # resolution answers `:eisdir` when the final component is a directory (clause
  # above) and `:enotdir` only when a non-final component is a regular file. So
  # this kind means a non-directory ancestor, and GNU words that "cannot stat":
  #
  #     $ cp a.md ./j/a.md          # j is a regular file
  #     cp: cannot stat './j/a.md': Not a directory
  #
  # Verified against GNU coreutils 9.x, not just BSD cp (which prints
  # "cp: ./j/a.md: Not a directory" — same condition, different shape).
